...
function CorrelationLine({ visible, correlation }: { visible: boolean, correlation: number }) {
    ...
    <div className="space-y-4">
      <div className="h-[400px] relative glass-card rounded-xl overflow-hidden border-2 border-gray-200">
        <Canvas camera={{ position: [0, 3, 8], fov: 50 }}>
          ...
          {/* Alice's detector */}
          <Detector position={[-3, 0, 0]} angle={angleA} color="#0ea5e9" label="Alice" />
          
          {/* Bob's detector */}
          <Detector position={[3, 0, 0]} angle={angleB} color="#d946ef" label="Bob" />
      ...
      {measuring && (
        <div className={`${Math.abs(correlation) > bellLimit ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'} border-2 rounded-lg p-4`}>
          <div className="space-y-2 text-sm text-gray-700">
            <p>
              <strong>Quantum Prediction:</strong> <span className="font-mono">{correlation.toFixed(3)}</span>
            </p>
            <p>
              <strong>Bell's Limit (1/√2):</strong> <span className="font-mono">±0.707</span>
            </p>
            <p>
              <strong>Angle Difference:</strong> {Math.round(((angleA - angleB) * 180) / Math.PI)}°
            </p>
            {Math.abs(correlation) > bellLimit ? (
              <p className="text-green-700 font-semibold pt-2 border-t border-green-300">
                ✓ Bell's inequality violated! Quantum mechanics wins! This correlation is impossible with local hidden variables.
              </p>
            ) : (
              <p className="text-orange-700 pt-2 border-t border-orange-300">
                Classical physics could explain this correlation. Try adjusting the angles to find violations!
              </p>
            )}
          </div>
        </div>
      ...
      <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-4 text-sm text-gray-700">
        ...
        <p>
          Try setting the angles 22.5° apart (like 0° and 22.5°, or 45° and 67.5°). These produce maximum 
          violations! Bell's limit of 0.707 can be exceeded up to about 0.854.
        </p>
      </div>
    </div>
  );
