...
export function BlochSphere() {
  ...
  const isMinusX =
    Math.abs(theta - Math.PI / 2) < 0.1 && Math.abs(phi - Math.PI) < 0.1;
  const isPlusY =
    Math.abs(theta - Math.PI / 2) < 0.1 && Math.abs(phi - Math.PI / 2) < 0.1;
  const isMinusY =
    Math.abs(theta - Math.PI / 2) < 0.1 &&
    Math.abs(phi - (3 * Math.PI) / 2) < 0.1;
    ...
    <div className="space-y-4">
      <div className="h-[500px] relative glass-card rounded-xl overflow-hidden border-2 border-gray-200">
        ...
        <div className="absolute top-4 left-4 bg-white/95 backdrop-blur-sm px-4 py-3 rounded-lg shadow-lg border border-gray-200">
          <p className="text-xs text-gray-600 mb-1">
            💡 Drag the green sphere!
          </p>
          <p className="text-sm font-semibold text-quantum-700">
            {stateDescription}
          </p>
        </div>
      </div>
      ...
      <div className="bg-purple-50 border-2 border-purple-200 rounded-lg p-4 text-sm text-gray-700">
        ...
        <ul className="space-y-1 text-xs">
          <li>
            <strong>North Pole (|0⟩):</strong> Qubit is definitely in state 0
          </li>
          <li>
            <strong>South Pole (|1⟩):</strong> Qubit is definitely in state 1
          </li>
          <li>
            <strong>Equator:</strong> Equal superposition with different phases
          </li>
          <li>
            <strong>Interior points:</strong> Various superposition states
          </li>
          <li>
            <strong>θ (theta):</strong> Controls probability mixture between |0⟩
            and |1⟩
          </li>
          <li>
            <strong>φ (phi):</strong> Controls the phase relationship (rotation
            around Z-axis)
          </li>
        </ul>
      </div>
